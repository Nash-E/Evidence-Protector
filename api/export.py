...
def export_csv(file_id):
    ...
    w.writerow(['Gap #', 'Severity', 'Score /100', 'Z-Score', 'Start Time', 'End Time', 'Duration', 'Start Line', 'End Line'])
    for g in gaps:
        w.writerow([
            g['id'], g['severity_label'], g['severity_score'], g['modified_z_score'],
            _fmt(g['start_time']), _fmt(g['end_time']), g['duration_human'],
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .findings-list {{ padding-left: 0; list-style: none; }}
...
<div class="container">
  ...
  <div class="stats-grid" style="margin-bottom:20px">
    <div class="stat"><div class="stat-num">{len(gaps)}</div><div class="stat-lbl">Gaps Detected</div></div>
    <div class="stat"><div class="stat-num">{meta.get('total_lines',0):,}</div><div class="stat-lbl">Lines Processed</div></div>
    <div class="stat"><div class="stat-num">{meta.get('malformed_count',0)}</div><div class="stat-lbl">Malformed Lines</div></div>
    <div class="stat"><div class="stat-num">{meta.get('out_of_order_count',0)}</div><div class="stat-lbl">Out-of-Order</div></div>
  </div>
  ...
  {oot_html}
  <div class="card">
    <div class="card-title">Detected Gaps</div>
  ...
  <div class="card">
    ...
    <table class="meta-table">
      ...
      <tr><td>Total log duration</td><td>{int(meta.get('total_log_seconds',0))}s</td></tr>
...
<div class="report-footer">Evidence Protector · Generated {now} · For investigative use only</div>
</body>
</html>"""
    return Response(
        html,
