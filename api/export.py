    ...
    for g in gaps:
        ...
        <div class="gap-card" style="border-left:4px solid {sc}">
          <div class="gap-card-header">
            <div class="gap-left">
              <span class="gap-num">Gap
              <span class="badge" style="background:{bg};color:{sc}">{g['severity_label']}</span>
              <span class="gap-duration">{g['duration_human']}</span>
            </div>
            <div class="gap-score" style="color:{sc}">{g['severity_score']}<span style="font-size:13px;font-weight:500;color:
          </div>
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .card-title {{ font-size: 16px; font-weight: 700; color:
  ...
  .verdict-text p {{ color:
  /* Stats grid */
  .stats-grid {{ display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; }}
  .stat {{ background:
  .stat-num {{ font-size: 24px; font-weight: 800; color:
  ...
  .findings-list li {{ padding: 9px 0; border-bottom: 1px solid
  .findings-list li:last-child {{ border-bottom: none; }}
  .actions-list li {{ display: flex; align-items: flex-start; gap: 12px; padding: 10px 0; border-bottom: 1px solid
  .actions-list li:last-child {{ border-bottom: none; }}
  .action-num {{ background:
  /* Gap cards */
  .gap-card {{ background:
  ...
  .gap-duration {{ font-size: 14px; font-weight: 600; color:
  .gap-score {{ font-size: 28px; font-weight: 800; }}
  .score-bar-wrap {{ background:
  .score-bar {{ height: 6px; border-radius: 999px; }}
  .gap-meta {{ display: flex; gap: 20px; font-size: 12px; color:
  .suggestions {{ background: white; border-radius: 8px; padding: 12px 16px; border: 1px solid
  .suggestions-title {{ font-size: 12px; font-weight: 700; color:
  .suggestions ul {{ padding-left: 18px; color:
  /* OOT */
  .oot-list {{ padding-left: 18px; color:
  /* MAD table */
  .meta-table {{ width: 100%; border-collapse: collapse; font-size: 13px; }}
  .meta-table td {{ padding: 8px 12px; border-bottom: 1px solid
  .meta-table td:first-child {{ color:
  .meta-table td:last-child {{ font-weight: 600; color:
  /* Footer */
  .report-footer {{ text-align: center; color:
  @media print {{
    body {{ background: white; }}
    .card {{ box-shadow: none; border: 1px solid
  }}
</style>
</head>
<body>
...
<div class="container">
  ...
  <!-- Severity pills -->
  <div class="pills">{pills_html if pills_html else '<span style="color:
  <!-- Key Findings -->
  <div class="card">
  ...
  <div class="card">
    ...
    {gap_rows_html if gap_rows_html else '<p style="color:
  </div>
  <!-- Technical Stats -->
  <div class="card">
